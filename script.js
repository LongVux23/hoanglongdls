...
const firebaseConfig = {
  apiKey: "AIzaSyCENQrOVNwirsGrZz-JXxXiUTbNTWBBtOU",
  authDomain: "hoanglong-dls-e9f38.firebaseapp.com",
  projectId: "hoanglong-dls-e9f38",
  storageBucket: "hoanglong-dls-e9f38.firebasestorage.app",
  messagingSenderId: "181944610474",
  appId: "1:181944610474:web:366a810495e0b6de81ead3"
};
...
function sendOrder() {
    ...
    // (XÓA BỎ localStorage và dùng biến allOrders toàn cục)
    allOrders.push(newOrder);
    db.collection("dls_data").doc("orders").set({ orderList: allOrders })
        .catch(error => console.error("Lỗi khi lưu đơn hàng:", error));
...
function renderAdminOrders() {
    const orders = allOrders; // GÁN BẰNG MẢNG TOÀN CỤC CỦA FIREBASE
...
function confirmOrder(index) {
    // Đổi trạng thái trong mảng toàn cục
    allOrders[index].status = "Đã xác nhận";
    
    // Cập nhật mảng mới lên Firebase
    db.collection("dls_data").doc("orders").set({ orderList: allOrders })
        .catch(error => alert("Lỗi khi cập nhật trạng thái: " + error.message));
        
    // Chú ý: Không cần gọi lại renderAdminOrders() nữa vì hàm onSnapshot (ở Bước 1) 
    // sẽ tự động nhận biết thay đổi và vẽ lại giao diện ngay lập tức!
}
